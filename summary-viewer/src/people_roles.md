...
const individualsWithTaskData = projectMembersDataFrame.map((individual) => {
  ...
  const allMetaDataForms = allTaskLogs.filter((log) => log.taskLogMetadata !== "No Metadata").length
...
const teamsWithTaskData = groupMembers.reduce((teamsMap, team) => {
  ...
  team.users.forEach((user) => {
    const memberName = `${user.firstName?.trim() || "No Name"} ${
      user.lastName?.trim() || "Provided"
